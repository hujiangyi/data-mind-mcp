...
对应的 Go 账号完成首次登录改密后，才能使用 MCP 配置。

- 全新服务使用 `admin` 和临时初始密码 `123456`；
- 管理员创建或重置的账号也使用 `123456`；
- 普通用户自助注册时可以设置初始密码，但第一次登录必须重新设置；
- 正式密码必须为 8～16 位。

首次改密未完成时，配置页面不会提供可用的 MCP 配置内容。请先登录
DataMind Go 网站完成改密，再重新打开邮件或安装链接。
